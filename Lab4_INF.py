...
def xmlFormatter():
    import re
    file = open('xml_1_string.txt', encoding='utf-8').readline().strip()
    tags = []
    tag = ''
    tmp = 0
    ...
    xml_file = open('xml_Main.txt', 'w', encoding='utf-8')
    for i in tags:
        if re.match(r'<schedule', i):
            ...
            xml_file.write('\t' * 2 + i + '\n')
        elif re.match(r'(<place)+|(<time)+|(<practician)+|(<format)+|(<trainer)', i + '\n'):
            xml_file.write('\t' * 3 + i + '\n')
        elif re.match(r'</lesson\d', i):
            xml_file.write('\t' * 2 + i + '\n')
        elif re.match(r'</day>', i):
...
def AdditionalTask1():
    ...
    import time
    starting = time.monotonic()
    json_file = open('json_Add1.txt', 'w', encoding='utf-8')
    tree = ET.parse('xml_Main.txt')
    root = tree.getroot()
...
def AdditionalTask2():
    ...
    import re
    starting = time.monotonic()
    json_file = open('json_Add2.txt', 'w', encoding='utf-8')
    json_file.write('{' + '\n')
    with open('xml_Main.txt', encoding='utf-8') as file:
        names = []
        count_of_options = 5
...
original = open('xml_1_string.txt', encoding='utf-8').readlines()
if len(original) == 1:
    xmlFormatter()
MainTask()
AdditionalTask1()
